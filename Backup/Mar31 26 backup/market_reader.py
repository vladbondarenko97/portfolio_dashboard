...
def analyze_market():
    ...
    # ==========================================
    # --- PILLAR 2: ADVANCED TECHNICALS ---
    # ==========================================
    try:
        spy_tech = yf.Ticker("SPY")
        # Pull 1 year of data to guarantee we can calculate the 200 SMA
        df_tech = spy_tech.history(period="1y")
        ...
        if df_tech.empty:
            ...
            ema_20 = df_tech['EMA_20'].iloc[-1]
            sma_50 = df_tech['SMA_50'].iloc[-1]
            sma_200 = df_tech['SMA_200'].iloc[-1]

            # --- 2. VOLUME PROFILE (30-Day POC) ---
            # Slice the last 30 trading days
            df_30 = df_tech.tail(30).copy()
            # Round prices to nearest $0.50 to cluster the volume
            df_30['Price_Zone'] = (df_30['Close'] * 2).round() / 2
            # Find the price zone with the absolute highest summed volume
            poc_price = df_30.groupby('Price_Zone')['Volume'].sum().idxmax()
            ...
            # Divergence Logic: Compare the peaks of the last 5 days vs the previous 15 days
            recent_high = df_tech['Close'].tail(5).max()
            prev_high = df_tech['Close'].iloc[-20:-5].max()
            
            recent_rsi_high = df_tech['RSI'].tail(5).max()
            prev_rsi_high = df_tech['RSI'].iloc[-20:-5].max()

            divergence_flag = "CLEAR (No Divergence)"
            # Tripwire: Price makes a higher high, but RSI makes a lower high, and RSI is historically overbought (>65)
            if recent_high > prev_high and recent_rsi_high < prev_rsi_high and prev_rsi_high > 65:
                divergence_flag = "🚨 BEARISH DIVERGENCE DETECTED (Higher Price Highs, Lower RSI Highs) 🚨"
            ...
            tech_payload = {
                ...
                "data": {
                    ...
                    "trend_matrix": {
                        "vs_20_EMA": "BULLISH" if current_price > ema_20 else "BEARISH",
                        "vs_50_SMA": "BULLISH" if current_price > sma_50 else "BEARISH",
                        "vs_200_SMA": "BULLISH" if current_price > sma_200 else "BEARISH"
                    },
                    "rsi_14d": round(current_rsi, 2),
                    "divergence_signal": divergence_flag
                }
            }
    ...
    # ==========================================
    # --- PILLAR 5: THE EXECUTION ENGINE (SCORING MATRIX) ---
    # ==========================================
    try:
        confluence_score = 0
        score_breakdown = []

        # 1. The Breadth Vote (Requires 'breadth_cond' from Pillar 4)
        if "BROAD RALLY" in locals().get('breadth_cond', ''):
            confluence_score += 1
            score_breakdown.append("Breadth: +1 (Broad Participation)")
        elif "NARROW RALLY" in locals().get('breadth_cond', '') or "BROAD SELLOFF" in locals().get('breadth_cond', ''):
            confluence_score -= 1
            score_breakdown.append("Breadth: -1 (Top-Heavy / Selloff)")
        else:
            score_breakdown.append("Breadth: 0 (Mixed/Neutral)")

        # 2. The Technical Vote (Requires 'current_price' and 'ema_20' from Pillar 2)
        cp = locals().get('current_price', 0)
        ema = locals().get('ema_20', 0)
        if cp > ema and ema != 0:
            confluence_score += 1
            score_breakdown.append("Tech (20 EMA): +1 (Above Trend)")
        elif cp < ema and ema != 0:
            confluence_score -= 1
            score_breakdown.append("Tech (20 EMA): -1 (Below Trend)")
        else:
            score_breakdown.append("Tech (20 EMA): 0 (On Trend / Unavailable)")

        # 3. The Gamma Vote (Requires 'call_wall_strike' and 'put_wall_strike' from Pillar 1)
        cw = locals().get('call_wall_strike', 0)
        pw = locals().get('put_wall_strike', 0)
        if cp > cw and cw != 0:
            confluence_score -= 1
            score_breakdown.append("Gamma: -1 (Overextended above Call Wall)")
        elif cp < pw and pw != 0:
            confluence_score += 1
            score_breakdown.append("Gamma: +1 (Bounce zone below Put Wall)")
        else:
            score_breakdown.append("Gamma: 0 (Within expected bounds)")

        # 4. The Max Pain Gravity (Requires 'max_pain_strike' from Pillar 1)
        mp = locals().get('max_pain_strike', 0)
        if cp > mp + 2 and mp != 0:
            confluence_score -= 1
            score_breakdown.append("Max Pain: -1 (Pulled Downward)")
        elif cp < mp - 2 and mp != 0:
            confluence_score += 1
            score_breakdown.append("Max Pain: +1 (Pulled Upward)")
        else:
            score_breakdown.append("Max Pain: 0 (Neutral Gravity)")
        ...
        if confluence_score >= 3 or confluence_score <= -3:
            if target_expiration != "NONE" and target_strike != "NONE":
                try:
                    ...
                    # 1. Validate Expiration (Account for market holidays shifting expirations to Thursday)
                    if target_expiration not in available_exps:
                        target_dt = datetime.strptime(target_expiration, "%Y-%m-%d")
                        # Find the absolute closest available expiration date
                        actual_exp = min(available_exps, key=lambda d: abs(datetime.strptime(d, "%Y-%m-%d") - target_dt))
                        expiration_rationale += f" (Adjusted to nearest available chain: {actual_exp})"
                    
                    # 2. Pull the specific options chain
                    ...
                    # 3. Validate Strike (Snap to nearest available if exact doesn't exist)
                    ...
                    if closest_strike != target_strike:
                        strike_rationale += f" (Adjusted from ${target_strike} to nearest listed: ${closest_strike})"
                        target_strike = closest_strike
                    
                    # 4. Extract the Live Contract Data
                    contract_row = opt_df[opt_df['strike'] == target_strike].iloc[0]
                    
                    contract_symbol = contract_row['contractSymbol']
                    last_price = contract_row['lastPrice']
                    bid = contract_row['bid']
                    ask = contract_row['ask']
                    iv = contract_row['impliedVolatility'] * 100
                    vol = contract_row['volume']
                    oi = contract_row['openInterest']
                    
                    # 5. Format the Trade Ticket
                    trade_ticket = {
                        "contract": contract_symbol,
                        "type": opt_type,
                        "strike": f"${target_strike}",
                        "expiration": actual_exp,
                        "pricing": f"Bid: ${bid:.2f} | Ask: ${ask:.2f} | Last: ${last_price:.2f}",
                        "metrics": f"IV: {iv:.2f}% | Volume: {vol} | Open Interest: {oi}"
                    }

                except Exception as e:
                    trade_ticket = f"Failed to pull live contract data from yfinance: {e}"
            else:
                trade_ticket = "Invalid parameters. Cannot generate ticket."
        else:
